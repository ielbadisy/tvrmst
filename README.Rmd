...
All functions use a time grid `t` and survival matrices `S` with dimension
`n_time x n_series`. Each column is a series (arm, group, or unit).
...
`rmst_dynamic()` returns RMST at every grid time (including 0) for each series.  
`rmst_delta_curve()` is the difference between two series in an `rmst_curve()` output.
...
`tvrmst_diff()` computes the difference in conditional tvRMST between two arms.
...
`bootstrap_tvrmst_diff_cols()` bootstraps by resampling columns (units).  
`bootstrap_tvrmst_diff_reps()` aggregates a list of replicate survival matrices.
