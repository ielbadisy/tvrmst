...
test_that("rmst_tau handles basic and edge cases", {
  t <- seq(0, 10, by = 1)
  S <- matrix(1, nrow = length(t), ncol = 2)

  out <- rmst_tau(t, S, tau = 5)
  expect_equal(out, c(5, 5))

  # tau beyond max(t) should extend with constant survival
  out2 <- rmst_tau(t, S, tau = 12)
  expect_equal(out2, c(12, 12))

  # tau before first positive time when t does not include 0
  t2 <- seq(1, 5, by = 1)
  S2 <- matrix(1, nrow = length(t2), ncol = 1)
  out3 <- rmst_tau(t2, S2, tau = 0.5)
  expect_equal(out3, 0.5)

  expect_error(rmst_tau(t, S, tau = -1))
})


test_that("rmst_curve and rmst_delta_curve behave as expected", {
  t <- seq(0, 4, by = 1)
  S <- cbind(rep(1, length(t)), rep(1, length(t)))
...
test_that("rmst_window integrates windowed differences", {
  t <- seq(0, 10, by = 1)
  S1 <- matrix(1, nrow = length(t), ncol = 1)
  S0 <- matrix(0.5, nrow = length(t), ncol = 1)
  s_grid <- seq(0, 5, by = 1)
  tau <- 2

  w <- rmst_window(t, S1, S0, s_grid, tau)
...
test_that("tvrmst_cond returns tau for constant survival", {
  t <- seq(0, 10, by = 1)
  S <- matrix(1, nrow = length(t), ncol = 1)
  s_grid <- seq(0, 8, by = 1)
  tau <- 3

  mu <- tvrmst_cond(t, S, s_grid, tau, eps = 1e-6)
...
test_that("tvrmst_diff is zero when arms are identical", {
  t <- seq(0, 8, by = 1)
  S <- matrix(exp(-0.2 * t), ncol = 1)
  s_grid <- seq(0, 6, by = 1)
  tau <- 2

  delta <- tvrmst_diff(t, S, S, s_grid, tau, eps = 1e-6)
...
test_that("plot helpers return ggplot objects when ggplot2 is available", {
  ...
  t <- seq(0, 4, by = 1)
  S0 <- matrix(exp(-0.2 * t), ncol = 1)
  S1 <- matrix(exp(-0.1 * t), ncol = 1)

  p1 <- plot_survival_curves(t, S0, S1)
  expect_s3_class(p1, "ggplot")

  s_grid <- seq(0, 3, by = 1)
  mu0 <- tvrmst_cond(t, S0, s_grid, tau = 1)
  mu1 <- tvrmst_cond(t, S1, s_grid, tau = 1)

  p2 <- plot_tvrmst(mu0, mu1)
  expect_s3_class(p2, "ggplot")

  delta <- tvrmst_diff(t, S1, S0, s_grid, tau = 1)
